...
export const getCustomerInvoices = async (
  ...
  try {
    ...
    const { rows, count } = await Invoices.findAndCountAll({
      where: { owner_id: userId, customer_id: customerId },
    ...
    sendResponse(response, 200, "Customer invoices fetched", {
      items: rows,
      pagination: {
        total: count,
        page,
 ...
 *     description: Returns all invoices for a specific customer owned by the authenticated user.
 *     tags:
 *       - Invoices
 *     security:
 *       - bearerAuth: []
 ...
 *     responses:
 *       200:
 *         description: Customer invoices fetched.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                 message:
 *                   type: string
 *                   example: Customer invoices fetched
 *                 error:
 *                   type: boolean
 *                   example: false
 *                 data:
 ...
 *                 message:
 *                   type: string
 *                   example: Customer Id is missing
 *                 error:
 *                   type: boolean
 *                   example: true
 *                 data:
